...
TAP_RMS_THRESHOLD = 0.01
MIN_SILENCE_BETWEEN_TAPS_S = 0.25
...
audio_buffer = np.zeros(int(SAMPLE_RATE * AUDIO_WINDOW_DURATION_S), dtype=np.float32)
last_tap_event_time = 0
processing_tap_features = False

# ------------------- AUDIO CALLBACK ------------------------
def audio_callback(indata, frames, time_info, status):
    global audio_buffer, last_tap_event_time, processing_tap_features
    if status:
        print(status, flush=True)
    new_audio_chunk = indata[:, 0].astype(np.float32)
    audio_buffer = np.concatenate((audio_buffer, new_audio_chunk))
    max_buffer_len = int(AUDIO_WINDOW_DURATION_S * SAMPLE_RATE)
    if len(audio_buffer) > max_buffer_len:
        audio_buffer = audio_buffer[-max_buffer_len:]
    current_time = time.monotonic()
    if not processing_tap_features and (current_time - last_tap_event_time > MIN_SILENCE_BETWEEN_TAPS_S):
        analysis_window_len = 1024
        if len(audio_buffer) >= analysis_window_len:
            rms = np.sqrt(np.mean(audio_buffer[-analysis_window_len:] ** 2))
            if rms > TAP_RMS_THRESHOLD:
                print(f"Audio Tap Detected! RMS: {rms:.4f} at {current_time:.3f}", flush=True)
                last_tap_event_time = current_time
                processing_tap_features = True
...
while True:
    ...
    if processing_tap_features and predicted_label:
